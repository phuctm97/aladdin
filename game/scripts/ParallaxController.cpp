...
ParallaxController::ParallaxController( ala::GameObject* gameObject, const std::string& name )
  : GameObjectComponent( gameObject, name ), _speed( 0.1f ), _selfTransform( NULL ) {}

void ParallaxController::setOffset( const ala::Vec2& offset ) {
  _offset = offset;
}

void ParallaxController::setSpeed( const float speed ) {
  _speed = speed;
}

void ParallaxController::onInitialize() {
...
void ParallaxController::onRender() {
  ...
  for ( size_t i = 0; i < _layerSizes.size(); ++i ) {
    ...
    auto d = cameraPosition.getX() * (i + 1) * _speed;
